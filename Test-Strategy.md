...
| **High**   | **IDE Settings Ignored:** The toggles in Rider's settings (e.g., "Enable Hot Reload") do not correctly control the feature, breaking the user's ability to manage their environment.       | With the app running, verify Hot Reload works. Go to `Settings > Build, Execution, Deployment > Hot Reload` and **disable** the "Enable Hot Reload" checkbox. Restart the application. Make a file change. **Expected:** Hot Reload is **inactive**. Re-enable the setting, restart, and verify functionality is restored.                                                                                                                                                                                                                                              |
...
1.  **Feedback protocol (top priority):** `dotnet watch` reports its state ("changes applied," "rude edit," etc.). Rider's UI is entirely dependent on correctly parsing this output.
...
Based on the the **Key Risks** (Section 3) and **System Analysis** (Section 4), I have structured the testing effort into a phased plan.
